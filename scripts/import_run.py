...
def main() -> None:
    ...
    include = ["checkpoints/generative_model_versions"]

    if ask_if_verbose("Download only last checkpoint?", default=True):
        last_ckpt = ls(f"{src}/checkpoints/generative_model_versions")[0]
        exclude.append("checkpoints/generative_model_versions/*")
        include.append(f"checkpoints/generative_model_versions/{last_ckpt}")
