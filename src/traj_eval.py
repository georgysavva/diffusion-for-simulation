...
class TrajectoryEvaluator:
    ...
    def evaluate_episode(
        ...
        obs = self._vae.encode(obs_img_norm).latent_dist.sample().mul_(0.18215)
        ...
        generated_trajectory_obs = torch.empty(
            obs.size(0) - self._num_seed_steps, *latent_shape, device=self._device
        )
        for step in tqdm(range(self._num_seed_steps, len(episode)), desc="Generating"):
            ...
            if auto_regressive:
                ...
                prev_obs[-1] = obs[step]
            generated_trajectory_obs[step - self._num_seed_steps] = generated_obs
        generated_trajectory_img = torch.cat(
            [
                obs_img[: self._num_seed_steps],
                denormalize_img(
                    self._vae.decode(generated_trajectory_obs / 0.18215).sample.clamp(
                        -1, 1
                    )
                ),
            ]
