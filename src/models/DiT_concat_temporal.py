...
from src.models.DiT_common import (
    ActionEmbedder,
    TimestepEmbedder,
...
class DiT(nn.Module):
    ...
    def forward(self, noised_obs, t, prev_obs, prev_act):
        ...
        x = einops.rearrange(
            ...
            "(b t) n m -> b t n m",
            b=t.shape[0],
            t=self.num_conditioning_steps + 1,
        )
        ...
        x = einops.rearrange(x, "b t n m -> (b n) t m")
        x += temporal
        x = einops.rearrange(
            x, "(b n) t m -> (b t) n m", b=t.shape[0], n=self.num_patches
        )
        ...
        noised_obs = self.unpatchify(noised_obs)  # (B, out_channels, H, W)
        return noised_obs
