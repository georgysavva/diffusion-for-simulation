...
class InnerModelConfig:
    ...
    num_actions: int  # set by trainer after env creation
...
class InnerModel(nn.Module):
    def __init__(self, cfg: InnerModelConfig) -> None:
        ...
        self.act_emb = nn.Sequential(
            nn.Embedding(
                cfg.num_actions, cfg.cond_channels // cfg.num_steps_conditioning
            ),
        ...
        self.conv_in = Conv3x3(
            (cfg.num_steps_conditioning + 1) * cfg.img_channels, cfg.channels[0]
        )
    ...
    def forward(
        ...
        if self.act_emb is not None:
            assert act.ndim == 2 or (
                act.ndim == 3
                and act.size(2) == self.act_emb[0].num_embeddings
                and set(act.unique().tolist()).issubset(set([0, 1]))
            )
            act_emb = (
                self.act_emb(act)
                if act.ndim == 2
                else self.act_emb[1]((act.float() @ self.act_emb[0].weight))
            )
        else:
            assert act is None
            act_emb = 0
